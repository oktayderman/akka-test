...
public class CsvImporter {
  ...
  //FLOW -> fist parameter(type of the data, second out parameter, materialize value)
  //materialize value -> may be emitted  from to Flow
  private Flow<ByteString, ByteString, NotUsed> lineDelimiter =
      Framing.delimiter(ByteString.fromString("\n"), 128,
              FramingTruncation.ALLOW);//last line without new line will be accepted also
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    Source<ByteString, CompletionStage<IOResult>> test = StreamConverters.fromInputStream(() -> Thread.currentThread().getContextClassLoader().getResourceAsStream("application.conf"));
    test.runForeach( byteString -> {
